...
string information() {
    string inf = "<<C>>Розробник: Карпіков Олег Сергійович";
    cout << inf << endl;
    return inf;
}
...
double s_calculation() {
    ...
    while(z == 0 || z < 0){
        cout << "Введiть z: ";
        if(z < 0 || z == 0){
            cout << "z не може бути  бо дорiвнювaти нулю.  ";
        }
        z = check(z);
    }
...
double calculation_salary () {
    ...
    cout << "Нарахована  сума  з рплатнi: " << salary << endl;
    tax1 = (salary * 15) / 100;
    cout << "Податок на  прибуток фiзособи: " << tax1 << endl;
    tax2 = (salary * 2) / 100;
    cout << "Податок до пенсiйного фонду: " << tax2 << endl;
    tax3 = (salary * 0.6) / 100;
    cout << "Податок до фонду з г льнообов'язкового держ вного соцi льного стр хування на  вип док безробiття: " << tax3 << endl;
    tax4 = salary / 100;
    cout << "Податок на  загальнообов'язкове держ не соцi льне стр хув ння у зв'язку з тимч совою втр тою пр цезд тностi: " << tax4 << endl;
    taxes = tax1 + tax2 + tax3 + tax4;
    cout << "Загальна  сума  до сплати податкiв: " << taxes << endl;
    salaryWithTaxes = salary - taxes;
    cout << "Кiнцева  сума  зарплатнi заподатками: " << salaryWithTaxes;
    return taxes;
    return salary;
}
int validationForSize() {
    ...
    while(i < 23 || i == 24 || i == 26 || i == 28 || i == 30 || i > 31) {
        if(i < 23 || i == 24 || i == 26 || i == 28 || i == 30 || i > 31 ) {
            cout << "Число повинне бути бiльше 0 та  не дорiвнювати числом 24,26,28,30 та  бiльше 31: ";
            cin.clear();
            fflush(stdin);
        }
        i = validationForSize();
...
int sizeOfSocks () {
    ...
    switch (sizeUkr) {
    case 23:
        cout << "Розмiр шкарпеток в Великобританiя, США м : 8" << endl;
        cout << "Розмiр шкарпеток в ЄС : 37/38" << endl;
        break;
    case 25:
        cout << "Розмiр шкарпеток в Великобританiя, США м : 9" << endl;
        cout << "Розмiр шкарпеток в ЄС : 39/40" << endl;
        break;
    case 27:
        cout << "Розмiр шкарпеток в Великобританiя, США м : 10" << endl;
        cout << "Розмiр шкарпеток в ЄС : 41/42" << endl;
        break;
    case 29:
        cout << "Розмiр шкарпеток в Великобританiя, США м : 11" << endl;
        cout << "Розмiр шкарпеток в ЄС : 43/44" << endl;
        break;
    case 31:
        cout << "Розмiр шкарпеток в Великобританiя, США м : 12" << endl;
